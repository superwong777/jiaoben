...
export function normalizeTelegramSource(input: string): { source: string; url: string } {
  ...
  if (/^-?\d+$/.test(value)) {
    throw new Error("Telegram 数字内部 ID 无法通过公开网页查询，请填写 @公开用户名或 t.me 链接")
  }
  ...
  if (/^https?:\/\//i.test(value)) {
    ...
    if (parts[0] === "joinchat" && parts[1]) {
      ...
      throw new Error("t.me/c 数字内部链接不是公开主页，请填写公开用户名或邀请链接")
    } else {
      value = parts[0]
    }
  }
...
export async function fetchTelegramAudience(input: string): Promise<TelegramAudience> {
  ...
  if (!countMatch) {
    ...
    throw new Error(title ? "该页面没有公开显示成员人数" : "找不到该频道/群组，或它不是公开可访问页面")
  }
